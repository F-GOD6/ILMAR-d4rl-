...
from cail.network import StateDependentPolicy,StateActionConcatedFunction,MetaStateIndependentPolicy,AdvantageStateActionFunction
...
class ILBQ(Algorithm):
    ...
    def __init__(
        ...
        self.qnet = StateActionConcatedFunction(
            state_shape=state_shape,
            action_shape=action_shape,
            hidden_units=units_critic,
            hidden_activation=nn.ReLU(inplace=True)
        ...
        self.optim_cost = Adam(self.cost.parameters(),lr=lr_critic)
        self.optim_qnet = Adam(self.qnet.parameters(), lr=lr_actor)
    ...
    def update(self, writer: SummaryWriter):
        ...
        expert_inputs_next = torch.cat([expert_states, expert_actions], -1)
        ...
        # nu learning
        q_expert = self.qnet(expert_inputs)
        q_union= self.critic(union_inputs)
        q_expert_next = self.critic(expert_next_states)
        union_nu= self.critic(union_states)
        q_union_next= self.critic(union_next_states)
    ...
    def update_critic(
        ...
        if self.learning_steps % 1000 == 0:
            ...
            print("meta loss:",meta_loss.item(),"simple_loss:",loss.item())
