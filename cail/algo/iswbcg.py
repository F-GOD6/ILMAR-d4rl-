...
from cail.network import StateDependentPolicy, TwinnedStateActionFunction , StateActionFunction , StateIndependentPolicy,StateActionConcatedFunction
...
class ISWBCG(Algorithm):
    ...
    def update(self, writer: SummaryWriter):
        ...
        union_pred = torch.concat([union_states, union_actions_pred], -1)
        expert_cost_val = self.critic(expert_inputs)
        union_cost_val= self.critic(union_inputs)                             #分别的专业度
        pred_cost_val = self.critic(union_pred)
        ...
        cost_pred = torch.nn.Sigmoid()(pred_cost_val)
        weight = (cost_prob / (1 - cost_prob)) - (cost_pred / (1 - cost_pred))
