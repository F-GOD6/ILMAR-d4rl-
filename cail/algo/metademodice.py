...
from cail.network import StateDependentPolicy, TwinnedStateActionFunction , StateActionFunction , StateIndependentPolicy,StateActionConcatedFunction,StateFunction,MetaStateDependentPolicy
...
class MetaDemoDICE(Algorithm):
    ...
    def update(self, writer: SummaryWriter,path):
        ...
        union_states,union_actions,union_next_states,init_states,_ = sample_from_dataset(self.buffer_union,self.batch_size,self.device)
        ...
        union_adv_nu = - union_cost + self.gamma * union_next_nu.detach() - union_nu.detach()
        ...
        pi_loss = - (weight * self.actor.evaluate_log_pi(union_states, union_actions)).mean()
        ...
        self.update_cost(cost_loss,meta_loss,writer)
    ...
    def update_cost(
        ...
        beta = 1
        final_loss = 0 * meta_loss + beta * loss
        if self.learning_steps % 1000 == 0:
            writer.add_scalar(
        ...
        if self.learning_steps % 1000 == 0:
            writer.add_scalar(
                'loss/cost', final_loss.item(), self.learning_steps)
...
class MetaDemoDICEExpert(Expert):
    """
    ...
    def __init__(
        ...
        super(MetaDemoDICEExpert, self).__init__(
            state_shape=state_shape,
            action_shape=action_shape,
            device=device
        )
