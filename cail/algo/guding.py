...
from cail.network import DeterministicPolicy, TwinnedStateActionFunction
...
class GC(Algorithm):
    ...
    def update_actor(self, states: torch.Tensor,actions: torch.Tensor,writer: SummaryWriter):
        ...
        writer: SummaryWriter
            writer for logs
        """
        pre = self.actor.sample(states)
        loss_actor = torch.nn.MSELoss()(pre,actions)
