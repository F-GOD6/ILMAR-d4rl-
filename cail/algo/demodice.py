...
from cail.network import StateDependentPolicy, TwinnedStateActionFunction , StateActionFunction , StateIndependentPolicy,StateActionConcatedFunction,StateFunction
from cail.buffer import SerializedBuffer
EPS = np.finfo(np.float32).eps
EPS2 = 1e-3
def minimax_discriminator_loss(expert_logits, union_logits, label_smoothing=0.0):
    """
    实现 Minimax 判别器损失函数。

    Args:
        expert_logits (torch.Tensor): 判别器对真实样本的输出 logits。
        union_logits (torch.Tensor): 判别器对生成样本的输出 logits。
        label_smoothing (float, optional): 应用于真实标签的平滑因子，默认为 0.0。

    Returns:
        torch.Tensor: 标量损失值。
    """
    # 确保 label_smoothing 在合理范围
    assert 0.0 <= label_smoothing < 1.0, "label_smoothing 必须在 [0, 1) 范围内。"

    # 计算真实标签的平滑值
    real_labels = torch.ones_like(expert_logits) * (1.0 - label_smoothing)
    fake_labels = torch.zeros_like(union_logits)  # 生成样本标签保持为 0

    # 定义二元交叉熵损失（带 logits）
    loss_fn = nn.BCEWithLogitsLoss()

    # 计算真实样本的损失
    loss_real = loss_fn(expert_logits, real_labels)
    # 计算生成样本的损失
    loss_fake = loss_fn(union_logits, fake_labels)
...
class DemoDICE(Algorithm):
    ...
    def update(self, writer: SummaryWriter):
        ...
        expert_states, expert_actions, expert_next_states,_ = sample_from_dataset(self.buffer_exp,self.batch_size,self.device)
        union_states,union_actions,union_next_states,init_states = sample_from_dataset(self.buffer_union,self.batch_size,self.device)
        ...
        union_cost_val= self.cost(union_inputs)                             #分别的专业度
        ...
        nu_inter = torch.concat([union_states, nu_inter,nu_next_inter], 0)             #似乎是乱整一批数据
        nu_inter.requires_grad_(True)
        nu_output = self.critic(nu_inter)         #真实值为无穷小虚假值无穷大
